...
class DiffusionTrainerLite(LightningLite):
    ...
    def create_and_save_sample_grid(self, model, size, x=None, context=None, latent_decoder=None, save_path=''):
        # constants
        N_PER_ROW = 2 # 这个是N_PER_BLOCK的因数
        PAD_VALUE = 1.0
        PADDING = 4
        N_PER_BLOCK = 4 # 这个和batchsize相等
        ...
        with torch.no_grad():
            ...
            if context is not None:
                ...
                samples_syn_cond = model.sample_ddim(N_PER_BLOCK, size, context=syn_context[:N_PER_BLOCK]).cpu()
                if latent_decoder is not None:
    ...
    def run(self, cfg):
        ...
        while True:
            ...
            for x, context in dataloader:
                ...
                loss = diffusion_model(x, context=context.float(), dropout_mask=dropout_mask)
                self.backward(loss)
                optimizer.step()
